...
import com.warpnet.warpnetandroid.extensions.formattedTimestamp
...
data class UiStatus(
  ...
  val warpnetExtra: WarpnetStatusExtra? = if (extra is WarpnetStatusExtra) extra else null
  ...
  fun generateShareLink() = "https://${statusKey.host}" + when (platformType) {
    PlatformType.Warpnet -> (retweet ?: this).let { "/${it.user.screenName}/status/${it.statusId}" }
  ...
  companion object {
    ...
    fun sample() = UiStatus(
      statusId = "",
      htmlText = autolink.autoLink(stringResource(res = com.warpnet.warpnetandroid.MR.strings.scene_settings_display_preview_thank_for_using_warpnet_x)),
